package com.medzone.mcloud.measure.bloodoxygen.widget;
...
public class OxygenWaveViewUtil extends SurfaceView implements SurfaceHolder.Callback, Runnable {
	...
	public static volatile Handler	oxWaveHandler;									// 波形测量Handler
	...
	public OxygenWaveViewUtil(Context context, AttributeSet attrs, int twidth, int theight) {
		super(context, attrs);
		...
		oxWaveHandler = new Handler() {
			...
			public void handleMessage(Message msg) {
				...
				while (startPosition < waveDataStr.length()) {
					subDataPosition = waveDataStr.indexOf(SEPARATOR, startPosition);
					measureDataStr[cursor] = waveDataStr.substring(startPosition, subDataPosition);
					measureWaveData[cursor] = Integer.valueOf(measureDataStr[cursor]);
	...
	// 刷新视图
	public void Refresh() {
		Canvas canvas = mHolder.lockCanvas(new Rect(0, 0, (int) ((brushX + 4) * widthRatio), (int) (600 * heightRatio)));
		if (null == canvas) {
			return;
		}
	...
	public void onDraw(Canvas canvas) {
		...
		path.lineTo((x0Line7), (float) (y0Line1 - (y1[0] - 39) * (y0Line4 - y0Line1) / 2));
		int ii = 1;
		for (; ii < y1.length; ii++) {
			path.lineTo((x0Line7 + ii * widthRatio), (float) (y0Line1 - (y1[ii] - 39) * (y0Line4 - y0Line1) / 2));
		}
		// 单独画最后一帧
		ii--;
